...
public class ReceitaView extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        SelecaoCategoriaComboBox1.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Selecione uma opção", "Sobremesa", "Aperetivo", "Bebida", "Salada", "Lanche", "Salgado", "Doce", "Almoço", "Janta" }));
    ...
    private void configureCategoryComboBox() {
        ...
        SelecaoCategoriaComboBox1.addItem("Aperitivo");
        SelecaoCategoriaComboBox1.addItem("Bebida");
        SelecaoCategoriaComboBox1.addItem("Salada");
        SelecaoCategoriaComboBox1.addItem("Lanche");
...
/**
     * Encontra um ingrediente pelo nome na lista de ingredientes disponíveis
     * @param nome Nome do ingrediente a ser encontrado
     * @return O modelo do ingrediente encontrado ou null se não existir
     */
    @SuppressWarnings("unused")
    private IngredienteModel encontrarIngredientePorNome(String nome) {
        if (nome == null || nome.isEmpty()) {
            return null;
        }
        return ingredientesDisponiveis.stream()
                .filter(ing -> ing.getNome().equalsIgnoreCase(nome))
                .findFirst()
                .orElse(null);
    }
    ...
    private void adicionarIngredienteReceita() {
        ...
        // Extrai o nome do ingrediente (remove a unidade de medida se existir)
        String nomeIngrediente = selecionado.split("\\s*\\([^)]*\\)")[0].trim();
        
        // Encontra o ingrediente na lista de disponíveis
        IngredienteModel ingrediente = encontrarIngredientePorNome(nomeIngrediente);
        
        // Se não encontrou no banco, tenta criar um temporário
        if (ingrediente == null) {
            ingrediente = new IngredienteModel();
            ingrediente.setNome(nomeIngrediente);
            
            // Tenta extrair a unidade de medida do texto selecionado
            if (selecionado.contains("(") && selecionado.contains(")")) {
                String unidade = selecionado.substring(selecionado.indexOf('(') + 1, selecionado.indexOf(')'));
                ingrediente.setUnidade_medida(unidade);
            } else {
                ingrediente.setUnidade_medida("un"); // Unidade padrão
            }
        }
        ...
        try {
            ...
            for (ReceitaIngredienteModel ri : ingredientesReceita) {
                if (ri.getIngrediente() != null && 
                    ri.getIngrediente().getNome().equalsIgnoreCase(ingrediente.getNome())) {
                    JOptionPane.showMessageDialog(this, 
            ...
            if (ingrediente.getId_ingrediente() > 0) {
                receitaIngrediente.setId_ingrediente(ingrediente.getId_ingrediente());
            }
    ...
    private void salvarReceita() {
        ...
        try {
            ...
            // Adicionar os ingredientes à receita
